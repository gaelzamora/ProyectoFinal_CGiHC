...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	if (key == GLFW_KEY_F)
	{
		theWindow->isPushing = true;
		theWindow->articulacion1 -= 10.0;
		theWindow->carPositionZ -= 0.2f;
		if (action == GLFW_RELEASE) {
			theWindow->isPushing = false;
		}
	}

	if (key == GLFW_KEY_G)
	{
		theWindow->isPulling = true;
		theWindow->articulacion1 += 10.0;
		theWindow->carPositionZ += 0.2f;
		if (action == GLFW_RELEASE) {
			theWindow->isPulling = false;
		}
	}
	...
	if (key == GLFW_KEY_H)
	{
		...
		{
			theWindow->articulacion3Direction = -1.0f;
		}
		else if (theWindow->articulacion3 <= 0.0f)
		{
			theWindow->articulacion3Direction = 1.0f;
		}
